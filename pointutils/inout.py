...
'''
Read ASCII formatted point clouds----------------------------------------------------------------------------------------------------
'''
def random_rgb():
    return np.random.randint(0, 255, 3)

def read_ascii_tree(fp):
    if (sys.version_info > (3, 0)):
        open_file = open(fp, encoding='ISO-8859-1', newline='\n' if sys.platform == 'win32' else None)
    else:
        open_file = open(fp)
    with open_file as ascii:
        arr = np.loadtxt(ascii, skiprows=1, usecols=(0, 1, 2))
        df = pd.DataFrame(data=arr[:, :3], columns=['x', 'y', 'z'])
    # Assign random RGB values to each data point
    df['red'], df['green'], df['blue'] = random_rgb()
    return df

def write_ascii_tree(output_name, pc, additional_fields=[], verbose=False):
    if verbose:
        print('Saving file:', output_name)
    cols = ['x', 'y', 'z'] + additional_fields
    with open(output_name, 'w') as ascii:
        ascii.write('x y z')
        for col in additional_fields:
            ascii.write(' ' + col)
        ascii.write('\n')
    with open(output_name, 'a') as ascii:
        pc[cols].to_csv(ascii, header=None, index=None, sep=' ')
    if verbose:
